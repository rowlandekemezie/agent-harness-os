...
workflow-bound tasks use version 6. Both require `EvaluationCompleted` between
validation and attempt completion and bind the selected profile's evaluation
policy, resolved task policy, and routing evidence to that decision. Older event
schema versions remain replayable without inventing newer evidence.
