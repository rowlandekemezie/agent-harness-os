...
and 2 reports remain readable. New task journals use event schema version 7;
versions 5 and 6 remain compatible. All three require `EvaluationCompleted`
between validation and attempt completion and bind the selected profile's
evaluation policy, resolved task policy, and routing evidence to that decision.
Older event schema versions remain replayable without inventing newer evidence.
